...
const resolvers = {
    ...
    Recipe: {
        ...
        tags: (parent) => knex('tags').join('recipe_tags', 'tags.id', '=', 'recipe_tags.tag_id').where('recipe_tags.recipe_id', parent.id).select('tags.*').orderBy('tags.name'),
        ...
        category: (parent) => knex('categories').where('id', parent.category_id).first()
    },
